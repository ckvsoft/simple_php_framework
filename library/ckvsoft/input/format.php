...
class Format
{
    ...
    public function __call($call, $unlimitedParams)
    {
        if (!function_exists($call))
            throw new \ckvsoft\CkvException(__CLASS__ . ": Invalid formatting: $call (Invalid Function)");
    ...
    {
        if (count($param) != 2)
            throw new \ckvsoft\CkvException(__FUNCTION__ . ': $param must have two values: find, replace');
    ...
    public function hash($str, $param)
    {
        if (!is_array($param) || count($param) > 2 || empty($param)) {
            throw new \ckvsoft\CkvException(__FUNCTION__ . ': $param must have one or two values (Encryption, Key (Optional))');
        }
